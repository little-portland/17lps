...
function Card({ href, title, meta='Explore', dark=false, active=false, onEnter, onFocus, onClick, style }:{
  ...
  return (
    <a href={href} className={`card ${dark?'is-dark':''} ${active?'is-active':''}`}
      ...
      <span className="card-meta">{meta} <span aria-hidden>→</span></span>
    </a>
  );
}
...
export default function ConceptPage() {
  ...
  const [dark,    setDark]    = useState('22:00');

  useEffect(()=>{
    let tt:number[]=[];
    const run=()=>{
      tt.forEach(clearTimeout); tt=[];
      setDining('20:00 / 20:30'); setDark('22:00');
      tt=[
        setTimeout(()=>setDining('18:40 / 19:10'),1800),
        setTimeout(()=>setDining('21:12 / 21:40'),1960),
        setTimeout(()=>setDining('19:55 / 20:14'),2120),
        setTimeout(()=>setDining('20:00 / 20:30'),2320),
        setTimeout(()=>setDark('23:17'),4600),
        setTimeout(()=>setDark('01:40'),4760),
        setTimeout(()=>setDark('21:52'),4920),
        setTimeout(()=>setDark('22:00'),5120),
      ];
    };
    run(); const iv=setInterval(run,9600);
    return()=>{clearInterval(iv);tt.forEach(clearTimeout);};
  },[]);
  ...
  useEffect(()=>{
    ...
    return()=>{clearTimeout(t);window.removeEventListener('scroll',onS);};
  },[]);

  useEffect(()=>{
    const els=document.querySelectorAll<HTMLElement>('.rs');
    const obs=new IntersectionObserver(ee=>{
      ee.forEach(e=>{ if(!e.isIntersecting)return; e.target.classList.add('iv'); obs.unobserve(e.target); });
  ...
  return (
    <>
          ...
          <div className="shell">
            <div className="sig" aria-hidden="true">
              ...
              <div className="sig-node sn-dark">
                <span className="sig-time">{dark}</span>
                <span className="sig-dot"><span className="sig-dot-fill"/></span>
              </div>
            </div>
      ...
      <style jsx global>{`
        html,body,#__next { margin:0; min-height:100%; background:${C.cream}; color:${C.ink}; font-family:${MONO}; -webkit-font-smoothing:antialiased; }
        body { overflow-x:hidden; }
        * { box-sizing:border-box; }
        ::-webkit-scrollbar { width:8px; }
        ::-webkit-scrollbar-track { background:rgba(28,28,26,0.1); }
        ::-webkit-scrollbar-thumb { background:${C.ink}; border:2px solid rgba(232,226,212,0.6); background-clip:content-box; }
        ::-webkit-scrollbar-thumb:hover { background:${C.pink}; border:2px solid rgba(232,226,212,0.6); background-clip:content-box; }
        ...
        .scene-nav-mobile.scene-nav--space,.scene-nav-mobile--space {
          background:rgba(232,226,212,.92)!important; backdrop-filter:blur(18px); -webkit-backdrop-filter:blur(18px);
        }
      ...
      <style jsx global>{`
        ...
        /* central axis — draws on page load */
        .axis-dawn {
        ...
        .monolith-stage::after {
          ...
          left:30%; right:30%; height:2px;
          background:radial-gradient(ellipse,rgba(28,28,26,.18),transparent 72%);
          filter:blur(6px);
          opacity:0;
        ...
        .venue-stage::before {
          content:''; position:absolute;
          left:9%; right:9%; bottom:12%; height:14%;
          border-radius:50%;
        ...
        .v-base { z-index:2; filter:saturate(.78) contrast(1.02) drop-shadow(0 16px 26px rgba(28,28,26,.1)) drop-shadow(0 32px 54px rgba(28,28,26,.08)); }
        ...
        .a-space.iv .vg-a { filter:drop-shadow(0 0 10px rgba(212,80,122,.28)) hue-rotate(-16deg) saturate(1.2); animation:glitchA 8.5s steps(1,end) 2s infinite; }
        .a-space.iv .vg-b { filter:drop-shadow(0 0 10px rgba(80,120,130,.2)) hue-rotate(18deg) saturate(1.1); animation:glitchB 8.5s steps(1,end) 2s infinite; }
        ...
        .card::before {
          ...
          background:linear-gradient(135deg,rgba(212,80,122,.14),transparent 42%),
        ...
        .card-title { display:block; font-size:clamp(17px,1.65vw,26px); font-weight:700; line-height:.95; letter-spacing:0; text-transform:uppercase; text-shadow:.012em 0 0 currentColor; position:relative; z-index:1; }
        .card-meta  { display:inline-flex; align-items:center; gap:4px; color:rgba(28,28,26,.56); font-size:10px; letter-spacing:.18em; text-transform:uppercase; position:relative; z-index:1; }
        ...
        @media(prefers-reduced-motion:reduce){
          ...
          .venue-wrap{ opacity:1; transform:none; }
          .sig-node{ opacity:1; transform:none; }
